...
El **gráfico arranca limpio**: por defecto solo dibuja el **conteo primario** (el más probable), a plena opacidad, sobre las velas con **EMA 50/200** finas de contexto. Las capas restantes se activan con toggles en la cabecera, agrupadas para no saturar la vista:

- **Alternativos** — añade los conteos alternativos atenuados (color con alpha, línea más fina, etiquetas tenues), para ver los otros escenarios sin perder la jerarquía.
- **Niveles** — VWAP anclado + soportes/resistencias + convergencia de objetivos.
- **Pivotes** — máximos/mínimos locales (fractales) marcados con triángulos + sus niveles proyectados como S/R (ver abajo).
- **Proyección** — la hipótesis fantasma de las ondas que faltan (ver abajo).

Una **leyenda** sobre el gráfico nombra cada color y estilo presente según los toggles activos (conteo primario con el color de su patrón, EMA 50/200, Fibonacci, y cuando corresponde Soporte/Resistencia/VWAP/Convergencia/Pivotes/Proyección), así no hay que adivinar qué es cada línea. Click en una tarjeta del panel aísla ese conteo; otro click vuelve a la vista general. Todos los escenarios siguen disponibles en el panel lateral aunque no se dibujen.
...
**Pivotes y niveles de pivote** (`domain/indicators/pivots.ts` + `features/chart/pivotOverlay.ts`, toggle "Pivotes"): detecta **swings** (máximos y mínimos locales) con un fractal simétrico estilo Williams —un máximo es una vela cuyo `high` supera al de las N velas a cada lado—. Los triángulos que se dibujan son solo los que **extienden la estructura de la tendencia** (estructura de mercado de Dow): en tendencia alcista, máximos y mínimos crecientes (rojo ▼ / verde ▲); en bajista, decrecientes. La tendencia se toma de la pendiente de la EMA50, así los retrocesos también muestran su estructura. Además, los precios de los swings recientes se proyectan como **rayos horizontales de soporte/resistencia** que **encuadran el precio actual** (los más cercanos por encima y por debajo), con la etiqueta de precio junto al eje. Es **anti-repaint**: un pivote solo se marca cuando tiene N velas cerradas a su derecha, así nunca se mueve al llegar velas nuevas. Confluencia y geometría, **no** una señal.

**Disparador de confirmación** (`domain/elliott/trigger.ts`): el plan de riesgo entra **anticipando** el giro, sobre una estructura que todavía no ha demostrado nada. La herramienta señala el **nivel de reacción más cercano en el sentido de la tesis** cuya ruptura sería la primera prueba a favor: se entra a peor precio y queda menos recorrido al objetivo, pero la operación deja de ser anticipada. Cuando ese mismo nivel invalida además un conteo alternativo de sesgo contrario, se indica — un solo movimiento confirma una lectura y descarta la opuesta. No inventa niveles (usa los S/R ya calculados) y no promete nada: romper un nivel reduce la contradicción, no la incertidumbre.

**Colchón de ATR en el stop** (opcional, selector en la calculadora): el nivel de invalidación suele coincidir con un soporte/resistencia muy tocado, que es justo donde el precio barre mechas antes de girar. Se puede alejar el stop del nivel `0,25× / 0,5× / 1× ATR` para que ese ruido no cierre la posición; la calculadora muestra **los dos precios** (el nivel que define la tesis y la salida real) y recalcula tamaño y R:R. Va **desactivado por defecto**: es un intercambio explícito (menos ruido a cambio de peor R:R), no una decisión que la herramienta tome por ti.

**Informe de situación** (`domain/report.ts` + `features/analysis/ReportDialog.tsx`, botón "Analizar situación"): reúne **en un solo texto** todo lo que la herramienta ya ha calculado para el par y la temporalidad que estés mirando — escenarios (principal y alternativos, con narrativa, invalidación, objetivo y relaciones Fibonacci), confluencia factor a factor, contexto (marco superior, régimen ADX/ATR, sentimiento, derivados), estructura (VWAP, soportes/resistencias, banda Fibonacci, convergencia), plan de riesgo con su checklist y la fiabilidad histórica del motor. Es una **agregación determinista**: no añade cálculos ni juicios nuevos, así que nunca "opina" más que la propia app, y el aviso de riesgo va siempre incluido. Se genera al pulsar (es una foto del momento) y se puede **copiar en Markdown** para pegarlo en el diario o en otra herramienta.
...
- `src/domain/indicators/` — RSI (Wilder), MACD (12/26/9), EMA (20/50/200), **flujo de volumen** (`obv.ts`, acumulado causal) y **ADX/percentil de ATR** (`adx.ts`, contexto de régimen) como funciones puras. El flujo usa el **delta real de agresión** de Binance (compras − ventas = `2·takerBuyVolume − volume`, mapeado de `k[9]` de las klines) cuando está disponible, con fallback al OBV clásico de Granville para velas sin ese dato.
- `src/domain/elliott/confluence.ts` — evalúa 8 factores para impulsos (5 para ABC, uno de ellos el **régimen**) que confirman o refutan el conteo. El score final del escenario es `0.5·estructura + 0.5·confluencia`.
- **Régimen de mercado** (`src/domain/indicators/adx.ts`): un chip de contexto (tendencia fuerte / rango / transición · compresión / expansión) desde el **ADX de Wilder** (fuerza de tendencia, sin dirección) y el **percentil de ATR** relativo sobre 150 velas (volatilidad comprimida/expandida). Es descriptivo: no anticipa dirección ni es señal. También entra como factor **suave** (`regimen`, peso 0.3) SOLO en correcciones ABC (baja tendencia o compresión son coherentes con una corrección); se omite en el warmup del indicador.
- **Convergencia de objetivos** (`src/domain/elliott/targetClusters.ts`): mediante un barrido de puntos detecta las **zonas donde apuntan a la vez varios de los conteos** dibujados (intersección de sus zonas objetivo/imanes de proyección). Se dibuja como banda índigo en el gráfico + fila en el panel, con el nº de conteos que convergen. Convergen por **geometría compartida** (mismos pivotes y ratios Fibonacci), no por confirmación independiente: es geometría, no probabilidad; no toca el score.
- **Checklist de disciplina** (`src/domain/checklist.ts` + diario): al guardar en el diario se captura y **CONGELA** un checklist de 5 condiciones (alineación con marco superior, confianza, VSA, derivados, R:R) y se deriva un **grado A/B/C**. El diario segmenta win rate / expectancy **por grado** (solo con ≥30 operaciones decididas). Es una foto de la disciplina de entrada contra el sesgo retrospectivo, **no una probabilidad ni una validación del método**; el grado nunca precarga el riesgo de la calculadora.
- **Lectura VSA de volumen** (`src/domain/elliott/vsa.ts`, integración de la capa medible de Wyckoff/Volume Spread Analysis): el factor de volumen lee **esfuerzo vs resultado** en el pivote de giro que Elliott ya detectó (fin de onda 5 / fin de onda C). Esfuerzo = volumen normalizado por **percentil/rank causal** sobre las velas anteriores (robusto al wash-trading de cripto: solo orden, no magnitud); resultado = CLV `(close−low)/(high−low)`. Clímax/absorción = esfuerzo alto **con** rechazo del extremo (predicado único en AND, no infla el disparo). Reemplaza la antigua heurística de volumen por medias; sigue siendo factor **suave** (mismas `key`/peso) y degrada en `wick_spike`. La divergencia de onda 5 (`div5`) se **corrobora con el flujo de volumen** (delta comprador−vendedor real; AND debilitante: solo puede bajar su frecuencia de disparo, nunca subirla), y el detalle del VSA muestra el **flujo agresor real** del giro junto al CLV. Es confluencia probabilística, no señal.
